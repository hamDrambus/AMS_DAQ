...
std::ofstream CaenFileWriterModule::FileGenerator::next() {
  ...
  return std::ofstream(ss.str(), m_file_format == FileFormat::Binary ? std::ios::binary : std::ios::out);
}

bool CaenFileWriterModule::FileGenerator::yields_unique(const std::string &pattern) {
...
void CaenFileWriterModule::configure() {
  ...
  m_buffer_size = getModuleSettings().value("buffer_size", 1 * daqutils::Constant::Mega);
...
void CaenFileWriterModule::start(unsigned run_num) {
  ...
  for (uint64_t chid = 0; chid < m_channels; chid++) {
    ...
    if (m_file_format == FileFormat::Binary) {
      std::get<ThreadContext>(it->second)
          .consumer.set_work(&CaenFileWriterModule::flusher, this, it->first,
                            std::ref(std::get<PayloadQueue>(it->second)), m_buffer_size,
                            FileGenerator(m_pattern, it->first, m_run_number, m_file_format));
    } else {
      std::get<ThreadContext>(it->second)
          .consumer.set_work(&CaenFileWriterModule::flusher_text, this, it->first,
                            std::ref(std::get<PayloadQueue>(it->second)), m_buffer_size,
                            FileGenerator(m_pattern, it->first, m_run_number, m_file_format));
    }
  }
...
void CaenFileWriterModule::flusher(const uint64_t chid, PayloadQueue &pq, const size_t max_buffer_size,
                               FileGenerator fg) const {
  addTag();
  size_t bytes_written = 0;
  std::ofstream out = fg.next();
  auto buffer = daqling::utilities::Binary();

  const auto flush = [&](daqling::utilities::Binary &data) {
  ...
  while (!m_stopWriters) {
    ...
    if (payload->size() + buffer.size() <= max_buffer_size) {
      buffer += daqling::utilities::Binary(payload->data(), payload->size());
    } else {
      buffer += daqling::utilities::Binary(payload->data(), payload->size());
      /*
...
void CaenFileWriterModule::flusher_text(uint64_t chid, PayloadQueue &pq, size_t max_buffer_size,
      FileGenerator fg) const {
  addTag();
  size_t bytes_written = 0;
  ...
  const auto flush = [&](EventDataType *data) {
    auto pos1 = out.tellp();
    ...
    m_channelMetrics.at(chid).bytes_written += pos2-pos1;
    ++events_written;
  ...
  while (!m_stopWriters) {
    ...
    if (events_written >= m_max_filesize) { // Rotate output files
      ERS_INFO(" Rotating output files for channel " << chid);
      //flush(buffer);
      out.flush();
      out.close();
